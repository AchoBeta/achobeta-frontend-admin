...
const open = ref<boolean>(false)// 父组件传来的数据
...
const inputBatch = ref<number>(0)// 处理drawer的逻辑
const showDrawer = () => {
  open.value = true
}
// 处理drawer的关闭逻辑
...
const onSave = (index: number) => {
  if (data.value[index].title.trim() === '') {
    ...
    // TODO: save data
    const postData: UpdateRecruitBatch = {
...
const handleOk = () => {
  if (inputTitle.value.trim() === '') {
    ...
    if (childRef.value) {
      ...
      // TODO: save data
      const postData: CreateRecruitBatch = {
              ...
              <div class="flex w-full justify-between items-center content-center">
                ...
                <div class="flex-row flex justify-center items-center ">
                  <FormOutlined
                    ...
                    @click="exitBatch(index)"
                  />
                  <a-switch
                    v-model:checked="item.isRun"
                    @click="shiftStatus(index)"
                  />
                </div>
              </div>
            </template>
