  ...
  <header class="flex mt-4 mx-4">
    ...
    <div class="flex-1">
      <a-select allow-clear :disabled="batchList?.length <= 0" v-model:value="condition.actId" placeholder="请选择活动"
        style="width: 90%" @change="getInterview">
          ...
          <a-card @click="null" :headStyle="{padding: '0 12px' }" :body-style="{padding: '12px', width: '100%'}"
            ...
            <div class="flex items-center justify-between w-full mb-4">
              ...
              <a-tag :color="INTERVIEW_STATUS[item?.status].color">{{
                INTERVIEW_STATUS[item?.status].name }}</a-tag>
            </div>
            <div>
              ...
              <div class="mb-1">邮箱: {{ item?.simpleStudentVO?.email }}</div>
            </div>

            <div class="text-md font-bold">综合得分：{{ item.sum || '暂未评分' }}</div>
          </a-card>
        </a-list-item>
      </template>
    </a-list>
