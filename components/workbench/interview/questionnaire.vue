...
const getQuestionnaire = () => {
  if (!props.participationId) return
...
<template>
  ...
  <a-list :loading="loading" :grid="{ gutter: 4, column: 1 }" :data-source="questionnaireDetail"
    ...
    <template #renderItem="{ item, index }">
      <a-list-item style="padding: 0 0; margin-bottom: 0" :key="item.id">
        <div class="rounded-xl p-4 ">
          ...
          <div class="mt-4 ml-4">
            <a-typography-paragraph :ellipsis="{rows: 4, expandable: true}" :content="item.answer || '--'" />
          </div>
        </div>
      </a-list-item>
    </template>
