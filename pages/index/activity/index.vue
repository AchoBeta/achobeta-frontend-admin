...
const editActivityId = ref<string | undefined>('')
...
const gradeList = ref(Array.from({ length: 10 }).fill(2018).map((item, index) => item + index + 1))
...
<template>
  <div class="p-4 bg-[#f5f5f5]">
    <a-page-header
      style="background-color: #fff; border: 1px solid rgb(235, 237, 240); padding-bottom: 12px;"
      title="活动管理"
      sub-title="管理某招新批次招新活动"
      :back-icon="false"
    >
      <template #footer>
        <a-tabs
  ...
  <a-modal
    ...
    :width="600"
    :title="editActivityId ? '活动编辑' : '创建活动'"
    :confirm-loading="loading"
    :footer="false"
    @cancel="() => modalVisible = false"
  >
    <a-alert
      message="活动开启时，不允许编辑活动"
      type="warning"
    />
    <a-form
      :ref="formRef"
    ...
    >
      ...
      <a-form-item
        label="简历状态"
        :name="['target', 'allMatch', 'status']"
        :rules="[{ required: false, message: '请选择简历状态' }]"
      >
        <a-select
          v-model:value="formState.target.allMatch.status"
          placeholder="请选择简历状态"
          mode="multiple"
        >
          <a-select-option
            v-for="(item, index) in Object.values(RESUME_STATUES)"
            :key="index"
            :value="item.value"
          >
            {{
      ...
      <a-form-item
        label="年级"
        :name="['target', 'anyMatch', 'grade']"
        :rules="[{ required: false, message: '请选择面向年级' }]"
      >
        <a-select
          v-model:value="formState.target.anyMatch.grade"
          placeholder="请选择面向年级"
          mode="multiple"
        >
          <a-select-option
      ...
      <a-form-item
        label="用户"
        :name="['target', 'anyMatch', 'userId']"
        :rules="[{ required: false, message: '请选择特殊用户' }]"
      >
        <a-select
          v-model:value="formState.target.anyMatch.userId"
          placeholder="请选择特殊用户"
          mode="multiple"
        >
          <a-select-option
