...
<template>
  <div class="p-4 bg-[#f5f5f5]">
    ...
    <a-table class="mt-4" :loading="loading" :columns="columns" :data-source="actInfo?.userParticipationVOS"
      :scroll="{ x: 1500 }">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'scheduleVOS'">
          <a-space :wrap="true" class="mb-2" v-for="(item,index) in record.scheduleVOS" :key="index">
        ...
        <template v-if="column.key === 'action'">
          <a-space>
            ...
            <a style="color: #1890ff" @click="openAppointModal(record)">预约</a>
          </a-space>
        </template>
    ...
    <a-modal :width="800" :open="appointVisible" title="创建面试预约" @cancel="closeAppointModal" @ok="createAppointment">
      <a-range-picker v-model:value="timeRange" show-time />
    </a-modal>
    <resume-editModal ref="resumeRef" />
    <interview-answerModal ref="answerRef" />
  </div>
