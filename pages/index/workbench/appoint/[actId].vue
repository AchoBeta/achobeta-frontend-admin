...
<template>
  <div class="p-4 bg-[#f5f5f5]">
    <a-page-header
      ...
      :title="actTitle"
      sub-title="管理员先创建面试预约，再根据面试预约创建面试(确定开启面试)"
      @back="navigateTo('/workbench')"
    >
      <template #extra>
        <a-select
          v-model:value="statuFilter"
          mode="multiple"
          style="width: 200px"
          placeholder="筛选简历状态"
      ...
      <div class="flex w-full mt-4">
        ...
        >
          <a-col
            v-for="(item, index) in actInfo?.timePeriodCountVOS"
            :key="index"
            class="mb-4"
            :span="8"
          >
    ...
    <a-table
      ...
      :data-source="actInfo?.userParticipationVOS"
      :scroll="{ x: 1500 }"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'gender'">
          <a-tag
            v-if="record.simpleStudentVO.gender === 1"
            color="pink"
          >
            女
          </a-tag>
        ...
        <template v-if="column.key === 'status'">
          <a-tag :color="RESUME_STATUES[record.simpleStudentVO.status].color">
            {{
              RESUME_STATUES[record.simpleStudentVO.status].name }}
          </a-tag>
        </template>

        <template v-if="column.key === 'timePeriod'">
          <a-space
            v-for="(item, index) in record.timePeriodVOS"
            :key="index"
            class="mb-2"
          >
        ...
        <template v-if="column.key === 'scheduleVOS'">
          <div
            v-for="(item, index) in record.scheduleVOS"
            :key="item.id"
            class="mb-4"
          >
            <a-space
              ...
              :wrap="true"
            >
              <a-tag :color="RESUME_STATUES[index].color">
                {{ item.startTime }}
              </a-tag>
              <a-tag :color="RESUME_STATUES[index].color">
                {{ item.endTime }}
        ...
        <template v-if="column.key === 'action'">
          <a-space>
            ...
            <a
              style="color: #1890ff"
              @click="openResumeModal(record)"
            >简历</a>
            <a
              style="color: #1890ff"
              @click="openAppointModal(record)"
            >预约管理</a>
          </a-space>
        </template>
      </template>
