...
import { INTERVIEW_STATUS, INTERVIEW_EVENT } from '~/constants/interview';
...
<template>
  <div style="background-color: #f5f5f5;">
    ...
    <a-anchor class="fixed w-40 top-20 right-10">
      <a-anchor-link v-for="item in anchorList" :key="item.key" :href="item.href" :title="item.title" />
    </a-anchor>

    <div class="fixed right-20 top-80">
      <a-affix v-for="item in Object.values(INTERVIEW_EVENT)" :key="item.key" class="mb-4">
        <a-button @click="handleInterviewEvent(item)" type="dashed">{{ item.name }}</a-button>
      </a-affix>
      <a-button @click="info" class="flex justify-center items-center ml-6" type="dashed" shape="circle">
        <template #icon>
    ...
    <div id="overview" class="w-[calc(100vw-27rem)] p-4">
      <div class="w-[90%] bg-white mx-auto p-4 rounded-lg">
        <!-- 面试概览 -->
        <div class="mb-12 pt-2">
          <a-page-header style="width: 100%" @back="() => navigateTo({path: '/workbench'})">
            <template #title>
              <div class="flex items-center">
                <div class="mr-4">{{ interviewDetail?.title }}</div>
        ...
        <!-- 简历 -->
        <a-divider id="resume" style="font-weight: bold; font-size: 20px; padding-top: 4rem;">简历</a-divider>
        <div v-if="interviewDetail?.simpleStudentVO.resumeId" style="margin: 50px 0;">
          <workbench-interview-resume :interviewId="interviewId"
            :resumeId="interviewDetail?.simpleStudentVO.resumeId" />
        </div>

        <!-- 问卷 -->
        <a-divider id="questionnaire" style="font-weight: bold; font-size: 20px; padding-top: 4rem;">问卷</a-divider>
        <div v-if=" interviewDetail?.scheduleVO.participationId" style="margin: 50px 0;">
          <workbench-interview-questionnaire :participationId="interviewDetail?.scheduleVO.participationId" />
        </div>

        <!-- 试卷 -->
        <a-divider id="paper" style="font-weight: bold; font-size: 20px; padding-top: 4rem;">参考试卷</a-divider>
        <div>
          <workbench-interview-paper :interviewId="interviewId" />
        </div>

        <!-- 评论区 -->
        <a-divider id="comment" style="font-weight: bold; font-size: 20px; padding-top: 4rem;">评论区</a-divider>
        <div>
          <workbench-interview-comment :interviewId="interviewId" />
        </div>

        <!-- 面试总结 -->
        <a-divider id="summary" style="font-weight: bold; font-size: 20px;
        ">面试总结</a-divider>
        <div>
          <workbench-interview-summary :interviewId="interviewId" />
        </div>
      </div>
    </div>
